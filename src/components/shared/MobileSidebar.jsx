          ...
          <div className="flex items-center gap-3 rounded-3xl bg-slate-100 p-4">
            <div className="relative">
              <button
                ...
                className="relative flex h-14 w-14 items-center justify-center overflow-hidden rounded-full border border-slate-300 bg-white text-lg font-semibold text-slate-900 transition hover:border-sky-400"
            ...
            <div>
              <p className="text-sm text-slate-500">{role === 'employee' ? 'Employee' : 'Admin'}</p>
              <p className="font-semibold text-slate-900">{currentUser?.name || currentUser?.displayName || currentUser?.email || 'User'}</p>
            </div>
          </div>
